...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, show_demo=False, save_data=None):
        ...
        if self.args.mixer == "qtran_base":
            ...
            if self.args.double_q:
                max_actions_current_ = torch.zeros(size=(batch.batch_size, batch.max_seq_length, self.args.n_agents, self.args.n_actions), device=batch.device)
                ...
                max_actions = torch.zeros(size=(batch.batch_size, batch.max_seq_length, self.args.n_agents, self.args.n_actions), device=batch.device)
    ...
    def load_models(self, path):
        ...
        self.optimiser.load_state_dict(torch.load("{}/opt.torch".format(path), map_location=lambda storage, loc: storage))
