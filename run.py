    ...
    while runner.t_env <= args.t_max:
        ...
        for _ in range(args.num_circle):
            if buffer.can_sample(args.batch_size):
                ...
                if args.is_batch_rl:
                    runner.t_env += int(torch.sum(episode_sample['filled']).cpu().clone().detach().numpy()) // args.batch_size
        ...
        if args.env == 'mmdp_game_1' and \
            ...
            for i in range(args.n_actions):
                for j in range(args.n_actions):
                    new_actions = torch.Tensor([i, j]).unsqueeze(0).repeat(args.episode_limit + 1, 1)
                    if i == 0 and j == 0:
                        rew = torch.Tensor([1, ])
                    else:
                        rew = torch.Tensor([0, ])
                    if i == 1 and j == 1:
                        new_obs = torch.Tensor([1, 0]).unsqueeze(0).unsqueeze(0).repeat(args.episode_limit, args.n_agents, 1)
                    else:
                        new_obs = torch.Tensor([0, 1]).unsqueeze(0).unsqueeze(0).repeat(args.episode_limit, args.n_agents, 1)
                    # Truncate batch to only filled timesteps
                    max_ep_t = episode_sample.max_t_filled()
                    episode_sample = episode_sample[:, :max_ep_t]
                    ...
                    new_actions_onehot = torch.zeros(episode_sample['actions'].squeeze(3).shape + (args.n_actions,))
        ...
        if (args.env == 'matrix_game_1' or args.env == 'matrix_game_2' or args.env == 'matrix_game_3') and \
            ...
            for i in range(args.n_actions):
                for j in range(args.n_actions):
                    ...
                    episode_sample = episode_sample[:, :max_ep_t]
                    episode_sample['actions'][0, :, :, 0] = new_actions
                    new_actions_onehot = torch.zeros(episode_sample['actions'].squeeze(3).shape + (args.n_actions,)).cuda()
